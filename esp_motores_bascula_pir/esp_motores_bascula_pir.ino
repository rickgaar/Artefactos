...
void setup() {
  ...
  Serial.println("...");
  balanza.set_scale(-523.85); // Establecemos la escala
...
void loop() {
  ...
  //Se mide la balanza cada 30 minutos
    if(millis() - readTime >= halfHour){
      ...
      if (difference > 0) { // Si hay diferencia positiva (se consumió comida)
...
void motorreductor(){
  ...
  if(millis() - t <= 53.25){
    ...
    bascula = balanza.get_units(20);
    if(bascula >= minimoBascula){
      t = 5053.25;
    }
  }else if(millis() - t <= 5106.5){
    digitalWrite(IN3, HIGH);
    digitalWrite(IN4, LOW);
    analogWrite(ENB, 255);
  }else if(millis() - t <= 10106.5){
    digitalWrite(IN3, LOW);
    digitalWrite(IN4, LOW);
    t = millis();
    bascula = balanza.get_units(20);
